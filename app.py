...
# Model setup
model = 'Fault_detect/last.onnx'
filename_classes = 'Fault_detect/detection_classes.txt'
...
class App(QWidget):
    ...
    def create_right_panel(self):
        ...
        down_splitter = QSplitter(Qt.Orientation.Vertical)
        down_splitter.addWidget(splitter)
    ...
    def identify_fault(self, frame, outs,
        ...
        try:
            ...
            QMessageBox.information(self, "Set confThreshold success", f"Threshold set to {confThreshold}")
        except ValueError:
            QMessageBox.warning(self, "Warning", "Invalid confThreshold input. Please enter a valid number.")
            return []

        nmsThreshold = self.nmsThreshold_input.text()
        try:
            nmsThreshold = float(nmsThreshold)
            QMessageBox.information(self, "Set confThreshold success", f"Threshold set to {nmsThreshold}")
        except ValueError:
            QMessageBox.warning(self, "Warning", "Invalid confThreshold input. Please enter a valid number.")
            return []
    ...
    def capture_and_save(self):
        if self.camera_on:
            ...
            if ret:
                ...
                fault_detect = self.identify_fault(frame, outs)
...
if __name__ == '__main__':
    ...
    with open("Fault_detect/styles.qss", 'r', encoding="utf-8") as file:
